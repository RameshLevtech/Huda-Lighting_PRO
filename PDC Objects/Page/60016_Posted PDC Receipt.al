page 60016 "Posted PDC Receipt"
{
    ...
    SourceTableView = SORTING(Code) WHERE(Posted = FILTER(true), Bounced = FILTER(false), "Returned/ Not Encashed" = FILTER(false));
            ...
            group("F&unctions")
            {
                ...
                action("&Apply Entries")
                {
                    ...
                    begin
                        ...
                        WITH PDCReceipt DO BEGIN
                                                    ...
                                                    REPEAT
                                                        ...
                                                        /*CustLedgEntry."Remaining Amount" :=
                                                          GenJnlPostLine.ExchAmount(
                                                            CustLedgEntry."Remaining Amount",
                                                            CustLedgEntry."Currency Code","Currency Code","Document Date");*/
                                                        //</LT>
                                                        CustLedgEntry."Remaining Amount" :=
                                                        ...
                                                        IF "Currency Code" = GLSetup."LCY Code" THEN BEGIN
                                                          ...
                                                          PDCAppliedAmt.Amount := CurrencyExcRate.ExchangeAmtLCYToFCY("Document Date",CustLedgEntry."Currency Code",
                                                                                  CustLedgEntry."Amount to Apply",CustLedgEntry."Original Currency Factor");
                                                        END ELSE BEGIN
                                                        */
                                                        PDCAppliedAmt.Amount := ApplyCustEntries.CalcApplnAmounttoApply(CustLedgEntry."Amount to Apply");
                                                        //<LT> Commented the code - PDC Upgrade
                                                        /*PDCAppliedAmt."Amount (LCY)" := GenJnlPostLine.ExchAmount(PDCAppliedAmt.Amount,"Currency Code",
                                                                                        GLSetup."LCY Code","Document Date");*/
                                                        //</LT>
                                                        //END;
                            ...
                            END;
                            /*
                             IF "Reference Type" = "Reference Type"::"1" THEN BEGIN
                               IF "Cheque No." = '' THEN
                ...
                action("&Return")
                {
                    ...
                    begin
                        ...
                        IF NOT (Status = Status::Deposited) THEN
                            EXIT;
    ...
    begin
        WITH PDCReceipt DO BEGIN
            ...
            //Window.INPUT(1, DateBounced);  //TempMS commented 
    ...
    begin
        WITH PDCReceipt DO BEGIN
            ...
            MODIFY;
            TESTFIELD("PDC Deposit Bank No.");
            //TESTFIELD("Return ProfitCenter");//TRI SAM
            GLSetup.GET();
            GLSetup.TESTFIELD("PDC Receipt Template");
            GLSetup.TESTFIELD("PDC Receipt Batch");
            Customer.GET("Customer No.");
            ...
            IF DateReturned = 0D THEN
                ERROR('Please Update Return Date');
            /*
            ...
            RecCLE.RESET;
            RecCLE.SETRANGE(RecCLE."Document No.", Code);
            IF RecCLE.FINDFIRST THEN BEGIN
                RecCLE1.GET(RecCLE."Closed by Entry No.");
                TgProfitCenter := RecCLE1."Global Dimension 1 Code";
            END;
            ...
            IF "Reference Type" = "Reference Type"::Invoice THEN BEGIN
                ...
                GenJnlLine.VALIDATE("Currency Code", "Currency Code");
                GenJnlLine.VALIDATE(Amount, -Amount);
                GenJnlLine.Description := 'PDC Returned';
                GenJnlLine."Source Code" := "Customer No.";
                //GenJnlLine."Cheque No." := "Cheque No.";
                GenJnlLine."Check No." := "Cheque No.";
                GenJnlLine.Validate("Payment Method Code", "Payment Method Code");
                //GenJnlLine.VALIDATE("Shortcut Dimension 1 Code","Shortcut Dimension 1 Code");//TRI SAM Code Commented
                GenJnlLine.VALIDATE("Shortcut Dimension 1 Code", TgProfitCenter);// TRI SAM
                GenJnlLine.VALIDATE("Shortcut Dimension 2 Code", "Shortcut Dimension 2 Code");
                GenJnlLine."PDC Entry" := TRUE;
                GenJnlLine.MODIFY;
                ...
                GenJnlLine."Document Date" := DateReturned;
                GenJnlLine.VALIDATE("Account Type", GenJnlLine."Account Type"::"G/L Account");
                GenJnlLine.VALIDATE("Account No.", '101960');
                GenJnlLine.VALIDATE("Currency Code", "Currency Code");
                GenJnlLine.VALIDATE(Amount, Amount);
                GenJnlLine.Description := 'PDC Returned';
                GenJnlLine."Source Code" := "Customer No.";
                ...
                GenJnlLine.Validate("Payment Method Code", "Payment Method Code");
                GenJnlLine.VALIDATE("Shortcut Dimension 1 Code", "Deposit Bank Dim1 Code");
                //GenJnlLine.VALIDATE("Shortcut Dimension 1 Code","Return ProfitCenter");
                GenJnlLine.VALIDATE("Shortcut Dimension 2 Code", "Deposit Bank Dim2 Code");
                GenJnlLine."PDC Entry" := TRUE;
                GenJnlLine.MODIFY;
                /*
            ...
            /*
             IF "Reference Type" = "Reference Type"::" " THEN BEGIN
               ...
               GenJnlLine."Document Date" := DateReturned;
               GenJnlLine.VALIDATE("Account Type",GenJnlLine."Account Type"::"G/L Account");
               GenJnlLine.VALIDATE("Account No.",'106090');
               GenJnlLine.VALIDATE("Currency Code","Currency Code");
               GenJnlTemplate.GET(GLSetup."PDC Receipt Template");
               GenJnlLine."Source Code" := GenJnlTemplate."Source Code";
               GenJnlLine.VALIDATE(Amount,Amount);
